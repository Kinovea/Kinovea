...
{ 
	...
	// Subject to be removed during refactoring:
		...
		private void BuildContextMenus()
		{
			...
			popMenuDrawings.Items.AddRange(new ToolStripItem[] { mnuConfigureDrawing, mnuConfigureFading, mnuSepDrawing2, mnuTrackTrajectory, mnuGotoKeyframe, mnuSepDrawing, mnuDeleteDrawing });
			#endregion
		...
		public void sldrSpeed_KeyDown(object sender, KeyEventArgs e)
		{
			...
			if (m_VideoFile.Loaded)
			{
				...
				{
					if (sldrSpeed.Value >= sldrSpeed.Minimum + sldrSpeed.SmallChange)
					{
				...
				{
					if (sldrSpeed.Value <= sldrSpeed.Maximum - sldrSpeed.SmallChange)
					{
						sldrSpeed.Value+=sldrSpeed.SmallChange;
					}
					e.Handled = true;
		...
		private void StretchSqueezeSurface()
		{
			...
			/*Size imageSize = new Size(m_DecodingSize.Width, m_DecodingSize.Height);
		...
		private void ImageResizerSE_MouseMove(object sender, MouseEventArgs e)
		{
			...
			{
				int iTargetHeight = (ImageResizerSE.Top - m_SurfaceScreen.Top + e.Y);
				int iTargetWidth = (ImageResizerSE.Left - m_SurfaceScreen.Left + e.X);
				ResizeByResizer(iTargetHeight, iTargetWidth);
			}
		}
		private void ImageResizerSW_MouseMove(object sender, MouseEventArgs e)
		{
			if (e.Button == MouseButtons.Left)
			{
				int iTargetHeight = (ImageResizerSW.Top - m_SurfaceScreen.Top + e.Y);
				int iTargetWidth = m_SurfaceScreen.Width + (m_SurfaceScreen.Left - (ImageResizerSW.Left + e.X));
				ResizeByResizer(iTargetHeight, iTargetWidth);
			}
		}
		private void ImageResizerNW_MouseMove(object sender, MouseEventArgs e)
		{
			if (e.Button == MouseButtons.Left)
			{
				int iTargetHeight = m_SurfaceScreen.Height + (m_SurfaceScreen.Top - (ImageResizerNW.Top + e.Y));
				int iTargetWidth = m_SurfaceScreen.Width + (m_SurfaceScreen.Left - (ImageResizerNW.Left + e.X));
				ResizeByResizer(iTargetHeight, iTargetWidth);
			}
		}
		private void ImageResizerNE_MouseMove(object sender, MouseEventArgs e)
		{
			if (e.Button == MouseButtons.Left)
			{
				int iTargetHeight = m_SurfaceScreen.Height + (m_SurfaceScreen.Top - (ImageResizerNE.Top + e.Y));
				int iTargetWidth = (ImageResizerNE.Left - m_SurfaceScreen.Left + e.X);
				ResizeByResizer(iTargetHeight, iTargetWidth);
			}
		}
		private void ResizeByResizer(int _iTargetHeight, int _iTargetWidth)
		{
			//-------------------------------------------------------------------
		...
		private int ShowNextFrame(Int64 _iSeekTarget, bool _bAllowUIUpdate)
		{
			...
			//m_DecodeWatch.Reset();
			//m_DecodeWatch.Start();
			int res = m_VideoFile.GetNextFrame((long)_iSeekTarget, m_iFramesToDecode);
		...
		private void SetPopupConfigureText(AbstractDrawing _drawing)
		{
					...
					if (e.Button == MouseButtons.Left)
					{
						...
						if (!m_bIsCurrentlyPlaying)
						{
							...
							else if (m_ActiveTool == DrawingToolType.Chrono)
							{
								...
								//m_Metadata.Chronos[m_Metadata.SelectedChrono].SetupColor(m_ColorProfile);
							}
							else
							{
								//-----------------------
								...
								if (m_ActiveTool != DrawingToolType.Text)
								{
									...
									//m_Metadata[m_iActiveKeyFrameIndex].Drawings[0].SetupColor(m_ColorProfile);
								}
								else
								{
					...
					else if (e.Button == MouseButtons.Right)
					{
						...
						if (!m_bIsCurrentlyPlaying)
						{
							...
							if (m_Metadata.IsOnDrawing(m_iActiveKeyFrameIndex, descaledMouse, m_iCurrentPosition))
							{
								...
								bool trackVisible = (ad is DrawingCross2D);
								...
								mnuTrackTrajectory.Visible = trackVisible;
								...
								mnuSepDrawing.Visible = trackVisible || gotoVisible;
								
								// "Color & Size" or "Color" depending on drawing type.
								SetPopupConfigureText(ad);
		...
		private void FlushOnGraphics(Bitmap _sourceImage, Graphics g, Size _iNewSize)
		{
			// 1. Image
		...
		private void OrganizeKeyframes()
		{
			...
			if (m_Metadata.Count > 0)
			{
				...
				foreach (Keyframe kf in m_Metadata.Keyframes)
				{
					KeyframeBox tBox = new KeyframeBox(kf);
					...
					tBox.UpdateTitle(kf.Title);
					
					tBox.Tag = iKeyframeIndex;
					tBox.pbThumbnail.SizeMode = PictureBoxSizeMode.CenterImage;
					
					tBox.CloseThumb += new KeyframeBox.CloseThumbHandler(ThumbBoxClose);
					tBox.ClickThumb += new KeyframeBox.ClickThumbHandler(ThumbBoxClick);
					tBox.ClickInfos += new KeyframeBox.ClickInfosHandler(ThumbBoxInfosClick);
		...
		private void EnableDisableKeyframes()
		{
			...
			for (int i = 0; i < pnlThumbnails.Controls.Count; i++)
			{
				...
				{
					// Enable Keyframes that are within Working Zone
					pnlThumbnails.Controls[i].Enabled = true;
					((KeyframeBox)pnlThumbnails.Controls[i]).pbThumbnail.Image = m_Metadata[i].Thumbnail;
					
					// New, we now show the title.
					//((ThumbBox)pnlThumbnails.Controls[i]).lblTimecode.Text = m_Metadata[i].Title;
				...
				{
					// Disable others
					pnlThumbnails.Controls[i].Enabled = false;
					((KeyframeBox)pnlThumbnails.Controls[i]).pbThumbnail.Image = m_Metadata[i].DisabledThumbnail;
...
/*
		...
		private string BuildFilename(string _FilePath, Int64 _position, TimeCodeFormat _timeCodeFormat)
		{
			...
			string suffix = TimeStampsToTimecode(_position - m_iSelStart, _timeCodeFormat, false);
			string maxSuffix = TimeStampsToTimecode(m_iSelEnd - m_iSelStart, _timeCodeFormat, false);
		...
		private bool IsSelectionAnalyzable()
		{
			...
			return m_VideoFile.IsSelectionAnalyzable(trkSelection.SelStart, trkSelection.SelEnd, m_PrefManager.WorkingZoneSeconds, m_PrefManager.WorkingZoneMemory);
			*/
			return false;
		}
		#endregion
