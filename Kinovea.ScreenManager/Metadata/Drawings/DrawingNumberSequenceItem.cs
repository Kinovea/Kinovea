...
{
    /// <summary>
    /// This is a single number in the number sequence.
    /// </summary>
    [XmlType("AutoNumber")]
    public class DrawingNumberSequenceItem : AbstractMultiDrawingItem, IKvaSerializable
    {
        #region Properties
        public string Name
        {
        ...
        public DrawingNumberSequenceItem(XmlReader xmlReader, PointF scale, TimestampMapper timestampMapper, Metadata metadata)
            : this(0, 0, Point.Empty, 0)
        {
             ReadXml(xmlReader, scale, timestampMapper);

             infosFading = new InfosFading(timestamp, metadata.AverageTimeStampsPerFrame);
             infosFading.UseDefault = false;
             infosFading.FadingFrames = 25;
        }
        #endregion
        ...
        public void ReadXml(XmlReader xmlReader, PointF scale, TimestampMapper timestampMapper)
        {
            ...
            while(xmlReader.NodeType == XmlNodeType.Element)
            {
                ...
                {
                    case "Time":
                        timestamp = timestampMapper(xmlReader.ReadElementContentAsLong());
                        break;
                    case "Location":
